...
RUN service postgresql restart && sleep 20; echo ". /var/www/mapit/mapit/virtualenv-mapit/bin/activate && INSERT INTO mapit_country (code, name) VALUES ('DEU', 'Germany');" | su -l -c "psql mapit" mapit
RUN which python
...
RUN service postgresql restart && sleep 20; echo ". /var/www/mapit/mapit/virtualenv-mapit/bin/activate && ALTER SEQUENCE mapit_area_id_seq RESTART WITH 10000;" | su -l -c "psql mapit" mapit
